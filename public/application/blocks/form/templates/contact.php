...
<div id="formblock<?php echo $bID; ?>" class="ccm-block-type-form">
	<form enctype="multipart/form-data" class="form-stacked miniSurveyView" id="miniSurveyView<?php echo $bID; ?>"
		...
		<div class="fields row">
			...
			<?php foreach ($questions as $ix => $question) { ?>
				<div class="<?php
				
				// BEGIN MOD
				// (I also added 'row' to class of parent container, above)
				if ($ix < 4) {
				    echo('col-12 col-sm-6 ');
				}
				// END MOD
				
				?>form-group field field-<?php echo $question['type']; ?> <?php echo isset($errorDetails[$question['msqID']]) ? 'has-error' : '' ?>">
